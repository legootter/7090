...
let dir="C:\\dev\\School\\7090\\Cura"
    ...
    for(let w=2;w<4;w+=1){//wall count
        for(const stl of stls){//object files
            for(let i=0;i<100;i+=1){//infill
                ...
                let command=`CuraEngine slice -v -p -j "${dir}\\defs\\prusa_i3.def.json" -s wall_line_count=${w} -s material_guid=${plaUUid} -s infill_pattern=${infill} -s infill_line_distance=${infillden} -l "C:\\dev\\School\\7090\\objects\\${stl}" 2>"${dir}\\outputraw\\${infill}\\infillType=${infill}&holeSize=${stl.split(".")[0]}&wallCount=${w}&infillDensity=${i}.txt"`
                setTimeout(()=>{
